...
class ExerciseEEGLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            feat = torch.cat((h_n[-2], h_n[-1]), dim=1)  # TODO-4: 拼接最后一层双向 hidden state
        else:
    ...
    def clip_gradients(self):
        # 梯度爆炸处理（训练侧）：backward 后执行
        return torch.nn.utils.clip_grad_norm_(self.parameters(), max_norm=5.0) # TODO-6: 使用 clip_grad_norm_ 裁剪 self.parameters()
